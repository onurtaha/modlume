...
export const metadata = { title: "Shaders" };
...
export default async function ShadersPage({
  ...
  try {
    result = await searchProjects(query, [["project_type:shader"]], sortBy, offset, LIMIT);
  } catch {
    error = true;
  }
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-12">
      <h1 className="text-3xl font-bold">Minecraft Shaders</h1>
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          {SORT_OPTIONS.map((opt) => (
            <Link
              key={opt.value}
              href={buildUrl("/shaders", { query, sort: opt.value })}
      ...
      {error ? (
        <p className="mt-12 text-center text-muted-foreground">
          Something went wrong loading shaders. Please try again later.
        </p>
        ...
        <p className="mt-12 text-center text-muted-foreground">No shaders found.</p>
      )}
      ...
      {totalPages > 1 && (
        <div className="mt-12 flex items-center justify-center gap-4">
          {page > 1 ? (
            <Link
              href={buildUrl("/shaders", { query, sort: sortBy, page: page - 1 })}
